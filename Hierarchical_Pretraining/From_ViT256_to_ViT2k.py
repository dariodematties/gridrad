...
import vision_transformer2k as vits2k
...
def get_args_parser():
    ...
    # Model 2k parameters
    parser.add_argument('--arch2k', default='vit2k_xs', type=str,
        choices=['vit2k_xs', 'vit_tiny', 'vit_small', 'vit_base', 'xcit', 'deit_tiny', 'deit_small'] \
                + torchvision_archs + torch.hub.list("facebookresearch/xcit:main"),
        help="""Name of architecture to train. For quick experiments with ViTs,
        we recommend using vit_tiny or vit_small.""")
    parser.add_argument('--patch_size2k', default=1, type=int, help="""Size in pixels
        of input square patches - default 16 (for 1x1 patches). Using smaller
        values leads to better performance but requires more memory. Applies only
        for ViTs (vit_tiny, vit_small and vit_base). If <16, we recommend disabling
        mixed precision training (--use_fp16 false) to avoid unstabilities.""")
    parser.add_argument('--out_dim2k', default=65536, type=int, help="""Dimensionality of
        the DINO head output. For complex and large datasets large values (like 65k) work well.""")
    parser.add_argument('--norm_last_layer2k', default=True, type=utils.bool_flag,
        help="""Whether or not to weight normalize the last layer of the DINO head.
        Not normalizing leads to better performance but can make the training unstable.
        In our experiments, we typically set this paramater to False with vit_small and True with vit_base.""")
    ...
    parser.add_argument("--pretrained_weights2k", default="", type=str, help="Path to pretrained weights to load.")
    parser.add_argument("--checkpoint_key2k", default="teacher", type=str, help='Key to use in the checkpoint (example: "teacher")')
    return parser
...
def inference_on_pretrained_model(args):
    ...
    # ============ building 2k model ============
    model_2k = load_2k_model(args)
    print(f"Model loaded: {args.arch2k} model with patch size {args.patch_size2k} and output dimension {args.out_dim2k}.")
    print(f"Model loaded: {model_2k}")
    ...
    for it, (images, filenames) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        # forward pass
        with torch.no_grad():
            features_cls4k = forward(images, model, args.device, model_2k, args.device)
            print(f"Features shape: {features_cls4k.shape}")
            output_features.append(features_cls4k)
    ...
    if utils.is_main_process():
        ...
        save_path = os.path.join(args.output_dir, "output_features_2k.pt")
...
def load_2k_model(args):
    # build model
    model = vits2k.__dict__[args.arch2k](
        patch_size=args.patch_size2k, num_classes=0
    )
    for p in model.parameters():
        p.requires_grad = False
    model.eval()
    ...
    try: 
        os.path.isfile(args.pretrained_weights2k)
        state_dict = torch.load(args.pretrained_weights2k, map_location="cpu", weights_only=False)
        if (
            args.checkpoint_key2k is not None
            and args.checkpoint_key2k in state_dict
        ):
            print(
                f"Take key {args.checkpoint_key2k} in provided checkpoint dict"
            )
            state_dict = state_dict[args.checkpoint_key2k]
        state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        # remove `backbone.` prefix induced by multicrop wrapper
        state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
        msg = model.load_state_dict(state_dict, strict=False)
...
def forward(x, model256, device256, model2k, device2k):
    """
    ...
    features_cls256 = features_cls256.transpose(1,2).transpose(1,3)
    # 5. [1 x 192], where 192 == dim of ViT_4K [ClS] token.
    features_cls256 = features_cls256.to(device2k, non_blocking=True)
    features_cls4k = model2k.forward(features_cls256)
    features_cls4k = features_cls4k.to('cpu')
