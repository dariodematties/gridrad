...
class TensorGaussianBlur(object):
    """
    Apply Gaussian Blur to the Tensor image with probability p.
    """
    def __init__(self, p=0.5, kernel_size=(3,3), sigma=(0.1, 2.0)):
        ...
        Args:
            ...
            sigma (tuple): range of sigma for the Gaussian kernel
        """
        self.prob = p
...
class TensorSolarization(object):
    """
    Apply Solarization to the Tensor image with probability p.
    Inverts all pixel values above a threshold value.
    """
    ...
    def __call__(self, x):
        if random.random() < self.p:
            # Create a mask of values above the threshold
...
#         Args:
#
#         - tensors (Tensor) [N, C, H, W]: tensors that contains images
...
#         sample = self.tensors[index]  # [C, H, W]
#         if self.transform:
...
class TensorDatasetInference(torch.utils.data.Dataset):
    def __init__(self, root, transform=None, ext='.pt'):
        """
        Args:
        - root (str): root directory of the dataset
    ...
    def __getitem__(self, idx):
        ...
        # Return both the data and the relative file path for disambiguation
        return data, os.path.relpath(self.files[idx], self.root)
